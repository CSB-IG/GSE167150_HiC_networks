...
# I cant believe ive been scripting in r like this 
# thesis deities have mercy lol 
#
# Main function to analyze all chromosomes
#
analyze_all_chromosomes <- function(normal_graphs, tnbc_graphs, output_dir = "results/igraph_network_analysis_results") {
...
main <- function() {
  ...
  # Load data (assuming the graphs are already in memory)
  # If not, uncomment and modify the following line:
  # load("your_graphs_data.RData")
...
# ##### create table (supp table1)
# comparison_table <- generate_network_comparison_table(normal_graphs, tnbc_graphs)
# 
# # Usage example:
# # comparison_table <- generate_network_comparison_table(normal_graphs, tnbc_graphs)
# write.csv(comparison_table, "results/igraph_intra/network_comparison_table_normal_tnbc_40000.csv", quote = FALSE, row.names = FALSE)


#### PLOTS
library(ggvenn)

t(comparison_table[,c("normal_edges_n", "tnbc_edges_n")]) -> edges_mat

colnames(edges_mat) <- chromosomes

par(oma=c(1,2,0,2))
barplot(edges_mat, beside=T, las=1)



## venn diagram 
comparison_table[,c(1, 4,5,6)] -> nodes_df

ggvenn(nodes_df)

ggvenn(list(Normal=seq(52:5400), TNBC=seq(1:5449)))
